...
from utils.data import ImageRecognitionDataset
...
class ImageRecognitionModel(nn.Module):
    def __init__(self, ortn, args, target, target_names, load_weights=False):
        super().__init__()
        self.ortn = ortn
        self.args = args
        self.target = target
        ...
        self.resnet.fc = nn.Linear(self.resnet.fc.in_features, len(target_names))
        if load_weights:
            self.load_state_dict(torch.load(os.path.join(self.output_dir, 'checkpoint.pth.tar')))

        self.to(args.device)

    def run_train(self, datasets: Dict[str, Dict[str, Dict[str, ImageRecognitionDataset]]]):
        optimizer = Adam(self.parameters(), lr=self.args.lr)
        datasets = {
            split: datasets[split][self.ortn][self.target]
            for split in ['train', 'val']
        }
        weight = datasets['train'].get_weight().to(self.args.device)
        data_loaders = {
            split: DataLoader(dataset, batch_size=self.args.batch_size, shuffle=split == 'train', collate_fn=dataset.collate_fn)
    ...
    def train_epoch(self, epoch, weight, data_loaders, optimizer):
        ...
        for split in ['train', 'val']:
            ...
            with torch.set_grad_enabled(training):
                for inputs, targets in tqdm(data_loader, ncols=80, desc=f'model {self.ortn} {self.target} {split} epoch {epoch}'):
    ...
    def run_test(self, datasets: Dict[str, Dict[str, Dict[str, ImageRecognitionDataset]]]):
        reporter = ClassificationReporter(self.output_dir, self.target_names)
        test_set = datasets['val'][self.ortn][self.target]
        test_loader = DataLoader(test_set, batch_size=self.args.batch_size, collate_fn=test_set.collate_fn)
        self.eval()
        with torch.no_grad():
            for inputs, targets in tqdm(test_loader, ncols=80, desc=f'testing {self.ortn} {self.target}'):
                inputs = inputs.to(self.args.device)
                targets = targets.to(self.args.device)
                logits = self.forward(inputs)
                for logit, target in zip(logits, targets):
                    reporter.append(logit, target.item())
